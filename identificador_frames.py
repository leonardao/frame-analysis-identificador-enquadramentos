...
def limpa_tweet(tweet_original):
    ...
    tweet = ' '.join (re.sub ( "(@[A-Za-z0-9]+)|(#[A-Za-z0-9]+)|(\w+:\/\/\S+)|(RT)", " ", tweet_original ).split () )

    # Elimina letras com acento e converte tudo em minúsculas.
    limpo = ''.join ( (frase for frase in unicodedata.normalize ( 'NFD', tweet ) if unicodedata.category ( frase ) != 'Mn') ).lower ()

    # Elimina puntuação.
    limpinho = re.sub ( '[' + string.punctuation + ']', '', limpo )
